...
- 以集合差计算未被计划保留 tag 引用的 digest，使仅由 stale tag 引用的 manifest 在删除前就进入同一不可变计划。
...
适配器负责 URL 编码、响应结构校验和把 API 字段转换为领域模型。分页地址必须与配置的 Hub API 同源，避免把 Bearer JWT 转发给不可信主机。HTTP 错误统一转换为不包含认证信息或响应正文的应用异常。
Hub API 的 GET 和认证 POST 遇到网络错误、429 或瞬时 5xx 时执行两次有界退避重试，并尊重不超过 30 秒的数值 `Retry-After`；DELETE 不重试，避免重放结果不确定的删除。
Docker Hub 偶尔会在不足 `page_size` 的末页后仍返回陈旧的 `next`，且该 URL 返回 404；仅在上一页明确不足请求大小时，适配器把这个 404 视为分页结束。首屏或满页后的 404 仍安全失败。
...
发现流程得到全部 digest 后，与公开 Hub API 返回且不在 stale tag 计划中的 digest 做差。Cookie 不进入领域模型，也不传递给 `crane`。
...
5. 输出完整 dry-run 计划。
6. apply 模式下先逐个删除 stale tag，再在单个临时 crane 会话中以最多 4 个 worker 删除同一轮无 tag digest。明确因其他 image 引用而失败的 manifest 会在同轮有删除进展时进入下一轮；没有进展时停止重试并报告失败。
...
- crane 测试使用临时目录与 fake runner，断言命令、标准输入、超时、凭据环境边界和 `DOCKER_CONFIG` 清理，不调用真实注册表。
